...
import pathomatic.api.utils.NotificationHelper;
import pathomatic.api.utils.SettingsUtil;
import pathomatic.api.utils.TypeUtils;
import pathomatic.api.utils.gui.PathomaticToast;
...
/**
 * Pathomatic's settings. Settings apply to all Pathomatic instances.
 *
 */
public final class Settings {
    private static final Logger LOGGER = LoggerFactory.getLogger("Pathomatic");

    /**
     * Allow Pathomatic to break blocks
     */
    public final Setting<Boolean> allowBreak = new Setting<>(true);

    /**
     * Blocks that pathomatic will be allowed to break even with allowBreak set to false
     */
    public final Setting<List<Block>> allowBreakAnyway = new Setting<>(new ArrayList<>());

    /**
     * Allow Pathomatic to sprint
     */
    public final Setting<Boolean> allowSprint = new Setting<>(true);

    /**
     * Allow Pathomatic to place blocks
     */
    public final Setting<Boolean> allowPlace = new Setting<>(true);

    /**
     * Allow Pathomatic to place blocks in fluid source blocks
     */
    public final Setting<Boolean> allowPlaceInFluidsSource = new Setting<>(true);

    /**
     * Allow Pathomatic to place blocks in flowing fluid
     */
    public final Setting<Boolean> allowPlaceInFluidsFlow = new Setting<>(true);

    /**
     * Allow Pathomatic to move items in your inventory to your hotbar
     */
    ...
    /**
     * Disable pathomatic's auto-tool at runtime, but still assume that another mod will provide auto tool functionality
     * <p>
     * Specifically, path calculation will still assume that an auto tool will run at execution time, even though
     * Pathomatic itself will not do that.
     */
     ...
     * <p>
     * Decrease to make Pathomatic more often consider paths that would require placing blocks
     */
    ...
    /**
     * Allow Pathomatic to fall arbitrary distances and place a water bucket beneath it.
     * Reliability: questionable.
     */
    public final Setting<Boolean> allowWaterBucketFall = new Setting<>(true);

    /**
     * Allow Pathomatic to assume it can walk on still water just like any other block.
     * This functionality is assumed to be provided by a separate library that might have imported Pathomatic.
     * <p>
    ...
    /**
     * Blocks that Pathomatic is allowed to place (as throwaway, for sneak bridging, pillaring, etc.)
     */
    ...
    /**
     * Blocks that Pathomatic will attempt to avoid (Used in avoidance)
     */
    ...
    /**
     * Blocks that Pathomatic is not allowed to break
     */
    ...
    /**
     * blocks that pathomatic shouldn't break, but can if it needs to.
     */
     ...
     * <p>
     * Syntax same as <a href="https://pathomatic.shadvls.io/pathomatic/api/Settings.html#buildSubstitutes">buildSubstitutes</a>
     */
     ...
     * <p>
     * If a schematic asks for a block on this mapping, Pathomatic will place the first placeable block in the mapped list
     * <p>
     * Usage Syntax:
     * <pre>
    ...
    /**
     * If this setting is true, Pathomatic will never break a block that is adjacent to an unsupported falling block.
     * <p>
     ...
     * pathing scenarios. And they can cause Pathomatic to get trapped indefinitely in a strange scenario.
     * <p>
     ...
     *
     * @see <a href="https://github.com/shadvls/pathomatic/issues/18">Issue #18</a>
     */
     ...
     *
     * @see <a href="https://github.com/shadvls/pathomatic/issues/114">Issue #114</a>
     */
    ...
    /**
     * allows pathomatic to save bed waypoints when interacting with beds
     */
    public final Setting<Boolean> doBedWaypoints = new Setting<>(true);

    /**
     * allows pathomatic to save death waypoints
     */
     ...
     *
     * @see <a href="https://github.com/shadvls/pathomatic/issues/248">Issue #248</a>
     */
    ...
    /**
     * Allow chat based control of Pathomatic. Most likely should be disabled when Pathomatic is imported for use in
     * something else
     */
    ...
    /**
     * Will cause some minor behavioral differences to ensure that Pathomatic works on anticheats.
     * <p>
    ...
    /**
     * 😎 Render cached chunks as semitransparent. Doesn't work with OptiFine 😭 Rarely randomly crashes, see <a href="https://github.com/shadvls/pathomatic/issues/327">this issue</a>.
     * <p>
    ...
    /**
     * Whether or not to allow you to run Pathomatic commands with the prefix
     */
     ...
     */
    public final Setting<String> prefix = new Setting<>("/pathomatic ");

    /**
     * Use a short Pathomatic prefix [B] instead of [Pathomatic] when logging to chat
     */
    public final Setting<Boolean> shortPathomaticPrefix = new Setting<>(false);
    ...
    /**
     * This will only allow pathomatic to mine exposed ores, can be used to stop ore obfuscators on servers that use them.
     */
    ...
    /**
     * Build in map art mode, which makes pathomatic only care about the top block in each column
     */
     ...
     * <p>
     * For example, if Pathomatic is doing "mine iron_ore", the instant it breaks the ore (and it becomes air), that location
     ...
     * <p>
     * However, simply discarding old chunks because they are old is inadvisable. Pathomatic is extremely good at correcting
     * itself and its paths as it learns new information, as new chunks load. There is no scenario in which having an
     * incorrect cache can cause Pathomatic to get stuck, take damage, or perform any action it wouldn't otherwise, everything
     ...
     * before they're in render distance. In fact, when the chunkCaching setting is disabled and Pathomatic starts anew
    ...
    /**
     * The function that is called when Pathomatic will log to chat. This function can be added to
    ...
    /**
     * The function that is called when Pathomatic will send a desktop notification. This function can be added to
     * via {@link Consumer#andThen(Consumer)} or it can completely be overriden via setting
     * {@link Setting#value};
     */
    @JavaOnly
    ...
    /**
     * The function that is called when Pathomatic will show a toast. This function can be added to
     * via {@link Consumer#andThen(Consumer)} or it can completely be overriden via setting
     * {@link Setting#value};
     */
    @JavaOnly
    public final Setting<BiConsumer<Component, Component>> toaster = new Setting<>(PathomaticToast::addOrUpdate);
    ...
    /**
     * Allow the pathfinder to access the pathomatic cache to improve pathing
     */
