...
public class NotificationHelper {
        ...
        if (SystemTray.isSupported()) {
            try {
                if (trayIcon == null) {
                    ...
                    trayIcon = new TrayIcon(image, "Pathomatic");
                    trayIcon.setImageAutoSize(true);
                    trayIcon.setToolTip("Pathomatic");
                    tray.add(trayIcon);
                }

                trayIcon.displayMessage("Pathomatic", text, error ? TrayIcon.MessageType.ERROR : TrayIcon.MessageType.INFO);
            } catch (Exception e) {
                e.printStackTrace();
            }
        } else {
    ...
    private static void mac(String text) {
        ...
        processBuilder.command("osascript", "-e", "display notification \"" + text + "\" with title \"Pathomatic\"");
        try {
    ...
    private static void linux(String text) {
        ProcessBuilder processBuilder = new ProcessBuilder();
        processBuilder.command("notify-send", "-a", "Pathomatic", text);
        try {
            processBuilder.start();
        } catch (IOException e) {
            e.printStackTrace();
