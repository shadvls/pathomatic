 ...
 * chat and console logging mechanisms, and the Pathomatic chat prefix.
 *
 * @since 8/1/2018
 */
public interface Helper {
     ...
     */
    GuiMessageTag MESSAGE_TAG = new GuiMessageTag(0xFF55FF, null, Component.literal("Pathomatic message."), "Pathomatic");

    static Component getPrefix() {
        // Inner text component
        final Calendar now = Calendar.getInstance();
        final boolean xd = now.get(Calendar.MONTH) == Calendar.APRIL && now.get(Calendar.DAY_OF_MONTH) <= 3;
        MutableComponent pathomatic = Component.literal(xd ? "Pathomatoe" : PathomaticAPI.getSettings().shortPathomaticPrefix.value ? "B" : "Pathomatic");
        pathomatic.setStyle(pathomatic.getStyle().withColor(ChatFormatting.LIGHT_PURPLE));

        // Outer brackets
        MutableComponent prefix = Component.literal("");
        prefix.setStyle(pathomatic.getStyle().withColor(ChatFormatting.DARK_PURPLE));
        prefix.append("[");
        prefix.append(pathomatic);
        prefix.append("]");
    ...
    default void logToast(Component title, Component message) {
        Minecraft.getInstance().execute(() -> PathomaticAPI.getSettings().toaster.value.accept(title, message));
    }
    ...
    default void logNotification(String message, boolean error) {
        if (PathomaticAPI.getSettings().desktopNotifications.value) {
    ...
    default void logNotificationDirect(String message, boolean error) {
        Minecraft.getInstance().execute(() -> PathomaticAPI.getSettings().notifier.value.accept(message, error));
    }
    ...
    default void logDebug(String message) {
        if (!PathomaticAPI.getSettings().chatDebug.value) {
    ...
    /**
     * Send components to chat with the [Pathomatic] prefix
     *
    ...
    default void logDirect(boolean logAsToast, Component... components) {
        ...
        if (!logAsToast && !PathomaticAPI.getSettings().useMessageTag.value) {
        ...
        if (logAsToast) {
            ...
            Minecraft.getInstance().execute(() -> PathomaticAPI.getSettings().logger.value.accept(component));
        }
    }
    ...
    default void logDirect(Component... components) {
        logDirect(PathomaticAPI.getSettings().logAsToast.value, components);
    }
    ...
    default void logDirect(String message, ChatFormatting color) {
        logDirect(message, color, PathomaticAPI.getSettings().logAsToast.value);
    }
    ...
    default void logDirect(String message) {
        logDirect(message, PathomaticAPI.getSettings().logAsToast.value);
    }

    default void logUnhandledException(final Throwable exception) {
        HELPER.logDirect("An unhandled exception occurred. " +
                        "The error is in your game's log, please report this at https://github.com/shadvls/pathomatic/issues",
                ChatFormatting.RED);
