...
/**
 * As set by ExamplePathomaticControl or something idk
 *
 */
public final class CustomGoalProcess extends PathomaticProcessHelper implements ICustomGoalProcess {
    ...
    public CustomGoalProcess(Pathomatic pathomatic) {
        super(pathomatic);
    }
    ...
    public void setGoal(Goal goal) {
        ...
        if (pathomatic.getElytraProcess().isActive()) {
            try {
                pathomatic.getElytraProcess().pathTo(goal);
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        switch (this.state) {
            ...
            case EXECUTING:
                ...
                if (this.goal == null || (this.goal.isInGoal(ctx.playerFeet()) && this.goal.isInGoal(pathomatic.getPathingBehavior().pathStart()))) {
                    onLostControl(); // we're there xd
                    if (Pathomatic.settings().disconnectOnArrival.value) {
                        ctx.world().disconnect();
                    }
                    if (Pathomatic.settings().notificationOnPathComplete.value) {
