...
import pathomatic.api.Settings;
import pathomatic.api.behavior.look.IAimProcessor;
import pathomatic.api.behavior.look.ITickableAimProcessor;
...
import pathomatic.process.ElytraProcess;
import pathomatic.utils.BlockStateInterface;
import pathomatic.utils.IRenderer;
import pathomatic.utils.PathRenderer;
import pathomatic.utils.accessor.IFireworkRocketEntity;
...
import static pathomatic.utils.PathomaticMath.fastCeil;
import static pathomatic.utils.PathomaticMath.fastFloor;
...
public final class ElytraBehavior implements Helper {
    ...
    public ElytraBehavior(Pathomatic pathomatic, ElytraProcess process, NetherPathfinderContext npf, BlockPos destination, boolean appendDestination) {
        ...
        if(ctx.world().dimension() == Level.NETHER) {
            this.pathFinder = Pathomatic.settings().elytraAllowAboveRoof.value && Pathomatic.settings().elytraAllowAboveBuildLimit.value
                    ? new BuildLimitPathFinder(ctx, npfContext)
                    : npfContext;
        } else {
            this.pathFinder = Pathomatic.settings().elytraAllowAboveBuildLimit.value ? new BuildLimitPathFinder(ctx, npfContext) : npfContext;
        }
    }
    ...
    public void onRenderPass(RenderEvent event) {
        ...
        final Settings settings = Pathomatic.settings();
        ...
        if (!this.blockedLines.isEmpty() && Pathomatic.settings().elytraRenderRaytraces.value) {
        ...
        if (this.simulationLine != null && Pathomatic.settings().elytraRenderSimulation.value) {
    ...
    public void onReceivePacket(PacketEvent event) {
        if (event.getPacket() instanceof ClientboundPlayerPositionPacket) {
            ctx.minecraft().execute(() -> {
                this.remainingSetBackTicks = Pathomatic.settings().elytraFireworkSetbackUseDelay.value;
            });
        }
    }

    public void pathTo() {
        if (!Pathomatic.settings().elytraAutoJump.value || ctx.player().isFallFlying()) {
    ...
    public void onTick() {
        ...
        if ((now - this.timeLastCacheCull) / 1000 > Pathomatic.settings().elytraTimeBetweenCacheCullSecs.value) {
            npfContext.queueCacheCulling(ctx.player().chunkPosition().x, ctx.player().chunkPosition().z, Pathomatic.settings().elytraCacheCullDistance.value);
    ...
    /**
     * Called by {@link pathomatic.process.ElytraProcess#onTick(boolean, boolean)} when the process is in control and the player is flying
     */
    public void tick() {
        ...
        pathomatic.getLookBehavior().updateTarget(solution.rotation, false);
        ...
        for (int relaxation = 0; relaxation < 3; relaxation++) { // try for a strict solution first, then relax more and more (if we're in a corner or near some blocks, it will have to relax its constraints a bit)
            ...
            for (int i = Math.min(playerNear + 20, path.size() - 1); i >= minStep; i--) {
                ...
                for (final Pair<Vec3, Integer> candidate : candidates) {
                    ...
                    final double minAvoidance = Pathomatic.settings().elytraMinimumAvoidance.value;
    ...
    private void tickUseFireworks(final Vec3 start, final Vec3 goingTo, final boolean isBoosted, final boolean forceUseFirework) {
        ...
        final boolean useOnDescend = !Pathomatic.settings().elytraConserveFireworks.value || ctx.player().position().y < goingTo.y + 5;
        ...
        final double elytraFireworkSpeed = Pathomatic.settings().elytraFireworkSpeed.value;
        if (this.remainingFireworkTicks <= 0 && (forceUseFirework || (!isBoosted
            ...
            if (!pathomatic.getInventoryBehavior().throwaway(true, ElytraBehavior::isBoostingFireworks) &&
                    !pathomatic.getInventoryBehavior().throwaway(true, ElytraBehavior::isFireworks)) {
    ...
    private final class SolverContext {
        ...
        public SolverContext(boolean async) {
            ...
            ITickableAimProcessor aim = ElytraBehavior.this.pathomatic.getLookBehavior().getAimProcessor().fork();
            if (async) {
    ...
    private boolean isHitboxClear(final SolverContext context, final Vec3 dest, final Double growAmount) {
        ...
        if (Pathomatic.settings().elytraRenderHitboxRaytraces.value) {
            boolean clear = true;
            for (int i = 0; i < 8; i++) {
    ...
    public boolean clearView(Vec3 start, Vec3 dest, boolean ignoreLava) {
        ...
        if (Pathomatic.settings().elytraRenderRaytraces.value) {
    ...
    private static FloatArrayList pitchesToSolveFor(final float goodPitch, final boolean desperate) {
        final float minPitch = desperate ? -90 : Math.max(goodPitch - Pathomatic.settings().elytraPitchRange.value, -89);
        final float maxPitch = desperate ? 90 : Math.min(goodPitch + Pathomatic.settings().elytraPitchRange.value, 89);
    ...
    private Pair<Float, Boolean> solvePitch(final SolverContext context, final Vec3 goal, final int relaxation) {
        ...
        final int ticks = desperate ? 3 : context.boost.isBoosted() ? Math.max(5, context.boost.getGuaranteedBoostTicks()) : Pathomatic.settings().elytraSimulationTicks.value;
    ...
    private void tickInventoryTransactions() {
        if (invTickCountdown <= 0) {
            ...
            if (r != null) {
                r.run();
                invTickCountdown = Pathomatic.settings().ticksBetweenInventoryMoves.value;
            }
        }
    ...
    private int findGoodElytra() {
        ...
        for (int i = 0; i < invy.size(); i++) {
            ...
            if (slot.getItem() == Items.ELYTRA && (slot.getMaxDamage() - slot.getDamageValue()) > Pathomatic.settings().elytraMinimumDurability.value) {
                return i;
            }
        }
        return -1;
    ...
    private void trySwapElytra() {
        if (!Pathomatic.settings().elytraAutoSwap.value || !invTransactionQueue.isEmpty()) {
            return;
        }

        ItemStack chest = ctx.player().getItemBySlot(EquipmentSlot.CHEST);
        if (chest.getItem() != Items.ELYTRA
                || chest.getMaxDamage() - chest.getDamageValue() > Pathomatic.settings().elytraMinimumDurability.value) {
            return;
        }
    ...
    void logVerbose(String message) {
        if (Pathomatic.settings().elytraChatSpam.value) {
            logDebug(message);
        }
    }
