...
import pathomatic.api.pathing.movement.IMovement;
import pathomatic.api.pathing.path.IPathExecutor;
...
import pathomatic.pathing.movement.movements.MovementFall;
import pathomatic.process.elytra.*;
...
public class ElytraProcess extends PathomaticProcessHelper implements IPathomaticProcess, IElytraProcess, AbstractGameEventListener {
    public State state;
    ...
    private ElytraProcess(Pathomatic pathomatic) {
        super(pathomatic);
        pathomatic.getGameEventHandler().registerEventListener(this);
    }

    public static IElytraProcess create(final Pathomatic pathomatic) {
        return NetherPathfinderContext.isSupported()
                ? new ElytraProcess(pathomatic)
                : new NullElytraProcess(pathomatic);
    }
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        try {
            final long seedSetting = Pathomatic.settings().elytraNetherSeed.value;
            if (seedSetting != this.behavior.npfContext.getSeed()) {
                logDirect("Nether seed changed, recalculating path");
                this.resetState();
            }
            if (predictingTerrain != Pathomatic.settings().elytraPredictTerrain.value && ctx.player().level().dimension() == Level.NETHER) {
                logDirect("elytraPredictTerrain setting changed, recalculating path from scratch");
                predictingTerrain = Pathomatic.settings().elytraPredictTerrain.value;
                this.resetState();
            }
            if (allowTight != Pathomatic.settings().elytraAllowTightSpaces.value) {
                logDirect("elytraAllowTightSpaces setting changed, recalculating path from scratch");
                allowTight = Pathomatic.settings().elytraAllowTightSpaces.value;
                this.resetState();
            }
            if (allowAboveBuildLimit != Pathomatic.settings().elytraAllowAboveBuildLimit.value) {
                logDirect("elytraAllowAboveBuildLimit setting changed, recalculating path from scratch");
                allowAboveBuildLimit = Pathomatic.settings().elytraAllowAboveBuildLimit.value;
                this.resetState();
            }
            if (allowAboveRoof != Pathomatic.settings().elytraAllowAboveRoof.value && ctx.player().level().dimension() == Level.NETHER) {
                logDirect("elytraAllowAboveRoof setting changed, recalculating path from scratch");
                allowAboveRoof = Pathomatic.settings().elytraAllowAboveRoof.value;
                this.resetState();
            }
        ...
        if (ctx.player().isFallFlying() && shouldLandForSafety()) {
            if (Pathomatic.settings().elytraAllowEmergencyLand.value) {
        ...
        if (ctx.player().isFallFlying() && this.state != State.LANDING && (this.behavior.pathManager.isComplete() || safetyLanding)) {
            ...
            if (last != null && ctx.player().position().distanceToSqr(last.getCenter()) < 1) {
                if (Pathomatic.settings().notificationOnPathComplete.value && !reachedGoal) {
                    logNotification("Pathing complete", false);
                }
                if (Pathomatic.settings().disconnectOnArrival.value && !reachedGoal) {
        ...
        if (this.state == State.LANDING) {
            ...
            if (ctx.player().isFallFlying() && endPos != null) {
                ...
                pathomatic.getLookBehavior().updateTarget(new Rotation(rotation.getYaw(), 0), false); // this will be overwritten, probably, by behavior tick
        ...
        if (this.state == State.FLYING || this.state == State.START_FLYING) {
            this.state = ctx.player().onGround() && Pathomatic.settings().elytraAutoJump.value
                    ? State.LOCATE_JUMP
                    : State.START_FLYING;
        }
        ...
        if (this.state == State.LOCATE_JUMP) {
            ...
            final IPathExecutor executor = pathomatic.getPathingBehavior().getCurrent();
            ...
            return new PathingCommandContext(this.goal, PathingCommandType.SET_GOAL_AND_PAUSE, new WalkOffCalculationContext(pathomatic));
        }
        ...
        if (this.state == State.START_FLYING) {
            if (!isSafeToCancel) {
                // owned
                pathomatic.getPathingBehavior().secretInternalSegmentCancel();
            }
            pathomatic.getInputOverrideHandler().clearAllKeys();
            if (ctx.player().fallDistance > 1.0f) {
                pathomatic.getInputOverrideHandler().setInputForceState(Input.JUMP, true);
            }
        }
        return new PathingCommand(null, PathingCommandType.CANCEL_AND_SET_GOAL);
    }
    ...
    private void destroyBehaviorAsync() {
        ...
        if (behavior != null) {
            this.behavior = null;
            Pathomatic.getExecutor().execute(() -> {
                behavior.destroy();
            });
        }
    }
    ...
    private void pathTo0(BlockPos destination, boolean appendDestination) {
        ...
        this.predictingTerrain = ctx.player().level().dimension() == Level.NETHER && Pathomatic.settings().elytraPredictTerrain.value;
        this.allowTight = Pathomatic.settings().elytraAllowTightSpaces.value;
        this.allowAboveBuildLimit = Pathomatic.settings().elytraAllowAboveBuildLimit.value;
        this.allowAboveRoof = Pathomatic.settings().elytraAllowAboveRoof.value;
        this.behavior = new ElytraBehavior(this.pathomatic, this, getNpfContext(), destination, appendDestination);
    ...
    private boolean isSupportedPos(BlockPos pos) {
        ...
        final int maxY = (isNether && !Pathomatic.settings().elytraAllowAboveRoof.value) ? 127 : Math.min(minY + 384, ctx.world().dimensionType().height() + minY);

        final boolean aboveRoof = Pathomatic.settings().elytraAllowAboveRoof.value;
        final boolean aboveBuild = Pathomatic.settings().elytraAllowAboveBuildLimit.value;
    ...
    private boolean shouldLandForSafety() {
        ...
        if (chest.getItem() != Items.ELYTRA || chest.getMaxDamage() - chest.getDamageValue() < Pathomatic.settings().elytraMinimumDurability.value) {
        ...
        if (qty <= Pathomatic.settings().elytraMinFireworksBeforeLanding.value) {
            return true;
        }
        return false;
    }
    ...
    public void onPostTick(TickEvent event) {
        IPathomaticProcess procThisTick = pathomatic.getPathingControlManager().mostRecentInControl().orElse(null);
    ...
    public static final class WalkOffCalculationContext extends CalculationContext {
        ...
        public WalkOffCalculationContext(IPathomatic pathomatic) {
            super(pathomatic, true);
    ...
    private static boolean isInBounds(Level dim, BlockPos pos) {
        ...
        int maxY = (dim.dimension() == Level.NETHER && !Pathomatic.settings().elytraAllowAboveRoof.value) ? 127 : Math.min(minY + 384, dimType.height() + minY);
        return pos.getY() >= minY && pos.getY() < maxY;
    }

    private boolean isSafeBlock(Block block) {
        return block == Blocks.NETHERRACK || block == Blocks.GRAVEL || block == Blocks.SOUL_SAND || block == Blocks.SOUL_SOIL || (block == Blocks.NETHER_BRICKS && Pathomatic.settings().elytraAllowLandOnNetherFortress.value)
    ...
    private NetherPathfinderContext getNpfContext() {
        if(this.npfContext == null) {
            ...
            this.npfContext = new NetherPathfinderContext(
                    Pathomatic.settings().elytraNetherSeed.value,
                    Pathomatic.settings().elytraUseCache.value ? pathomatic.getWorldProvider().getCurrentWorld().directory.resolve("cache") : null,
                    ctx.world()
            );
        }
