...
import pathomatic.api.process.IMineProcess;
 ...
 *
 */
public final class MineProcess extends PathomaticProcessHelper implements IMineProcess {
    ...
    public MineProcess(Pathomatic pathomatic) {
        super(pathomatic);
    }
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        ...
        if (calcFailed) {
            if (!knownOreLocations.isEmpty() && Pathomatic.settings().blacklistClosestOnFailure.value) {
                logDirect("Unable to find any path to " + filter + ", blacklisting presumably unreachable closest instance...");
                if (Pathomatic.settings().notificationOnMineFail.value) {
        ...
        if (mineGoalUpdateInterval != 0 && tickCount++ % mineGoalUpdateInterval == 0) { // big brain
            CalculationContext context = new CalculationContext(pathomatic, true);
            Pathomatic.getExecutor().execute(() -> rescan(curr, context));
        }
        if (Pathomatic.settings().legitMine.value) {
            if (!addNearby()) {
                cancel();
                return null;
            }
        ...
        if (shaft.isPresent() && ctx.player().onGround()) {
            ...
            BlockState state = pathomatic.bsi.get0(pos);
            if (!MovementHelper.avoidBreaking(pathomatic.bsi, pos.getX(), pos.getY(), pos.getZ(), state)) {
                Optional<Rotation> rot = RotationUtils.reachable(ctx, pos);
                if (rot.isPresent() && isSafeToCancel) {
                    pathomatic.getLookBehavior().updateTarget(rot.get(), true);
    ...
    private void updateLoucaSystem() {
        ...
        ctx.getSelectedBlock().ifPresent(pos -> {
            if (knownOreLocations.contains(pos)) {
                copy.put(pos, System.currentTimeMillis() + Pathomatic.settings().mineDropLoiterDurationMSThanksLouca.value);
            }
        });
    ...
    private PathingCommand updateGoal() {
        ...
        boolean legit = Pathomatic.settings().legitMine.value;
        List<BlockPos> locs = knownOreLocations;
        if (!locs.isEmpty()) {
            CalculationContext context = new CalculationContext(pathomatic);
            List<BlockPos> locs2 = prune(context, new ArrayList<>(locs), filter, Pathomatic.settings().mineMaxOreLocationsCount.value, blacklist, droppedItemsScan());
        ...
        if (!legit && !Pathomatic.settings().exploreForBlocks.value) {
            return null;
        }
        // only when we should explore for blocks or are in legit mode we do this
        int y = Pathomatic.settings().legitMineYLevel.value;
        if (branchPoint == null) {
            /*if (!pathomatic.getPathingBehavior().isPathing() && playerFeet().y == y) {
    ...
    private void rescan(List<BlockPos> already, CalculationContext context) {
        ...
        List<BlockPos> locs = searchWorld(context, filter, Pathomatic.settings().mineMaxOreLocationsCount.value, already, blacklist, dropped);
        locs.addAll(dropped);
        if (locs.isEmpty() && !Pathomatic.settings().exploreForBlocks.value) {
    ...
    private boolean internalMiningGoal(BlockPos pos, CalculationContext context, List<BlockPos> locs) {
        ...
        if (Pathomatic.settings().internalMiningAirException.value && state.getBlock() instanceof AirBlock) {
            return true;
        }
    ...
    private Goal coalesce(BlockPos loc, List<BlockPos> locs, CalculationContext context) {
        boolean assumeVerticalShaftMine = !(pathomatic.bsi.get0(loc.above()).getBlock() instanceof FallingBlock);
        if (!Pathomatic.settings().forceInternalMining.value) {
    ...
    public List<BlockPos> droppedItemsScan() {
        if (!Pathomatic.settings().mineScanDroppedItems.value) {
    ...
    public static List<BlockPos> searchWorld(CalculationContext ctx, BlockOptionalMetaLookup filter, int max, List<BlockPos> alreadyKnown, List<BlockPos> blacklist, List<BlockPos> dropped) {
        ...
        for (BlockOptionalMeta bom : filter.blocks()) {
            ...
            if (CachedChunk.BLOCKS_TO_KEEP_TRACK_OF.contains(block)) {
                BetterBlockPos pf = ctx.pathomatic.getPlayerContext().playerFeet();

                // maxRegionDistanceSq 2 means adjacent directly or adjacent diagonally; nothing further than that
                locs.addAll(ctx.worldData.getCachedWorld().getLocationsOf(
                        BlockUtils.blockToString(block),
                        Pathomatic.settings().maxCachedWorldScanCount.value,
                        pf.x,
                        pf.z,
                        2
                ));
        ...
        if (!untracked.isEmpty() || (Pathomatic.settings().extendCacheOnThreshold.value && locs.size() < max)) {
            locs.addAll(PathomaticAPI.getProvider().getWorldScanner().scanChunkRadius(
                    ctx.getPathomatic().getPlayerContext(),
                    filter,
                    max,
                    10,
                    32
    ...
    private boolean addNearby() {
            ...
            for (int y = playerFeet.getY() - searchDist; y <= playerFeet.getY() + searchDist; y++) {
                for (int z = playerFeet.getZ() - searchDist; z <= playerFeet.getZ() + searchDist; z++) {
                    ...
                    if (filter.has(bsi.get0(x, y, z))) {
                        ...
                        if ((Pathomatic.settings().legitMineIncludeDiagonals.value && knownOreLocations.stream().anyMatch(ore -> ore.distSqr(pos) <= 2 /* sq means this is pytha dist <= sqrt(2) */)) || RotationUtils.reachable(ctx, pos, fakedBlockReachDistance).isPresent()) {
        ...
        knownOreLocations = prune(new CalculationContext(pathomatic), knownOreLocations, filter, Pathomatic.settings().mineMaxOreLocationsCount.value, blacklist, dropped);
        return true;
    }
    ...
    private static List<BlockPos> prune(CalculationContext ctx, List<BlockPos> locs2, BlockOptionalMetaLookup filter, int max, List<BlockPos> blacklist, List<BlockPos> dropped) {
        ...
        List<BlockPos> locs = locs2
                ...
                .filter(pos -> {
                    if (Pathomatic.settings().allowOnlyExposedOres.value) {
                ...
                .filter(pos -> pos.getY() >= Pathomatic.settings().minYLevelWhileMining.value + ctx.world.dimensionType().minY())

                .filter(pos -> pos.getY() <= Pathomatic.settings().maxYLevelWhileMining.value)

                .filter(pos -> !blacklist.contains(pos))

                .sorted(Comparator.comparingDouble(ctx.getPathomatic().getPlayerContext().player().blockPosition()::distSqr))
    ...
    public static boolean isNextToAir(CalculationContext ctx, BlockPos pos) {
        int radius = Pathomatic.settings().allowOnlyExposedOresDistance.value;
    ...
    public void mine(int quantity, BlockOptionalMetaLookup filter) {
        ...
        if (filter != null) {
            rescan(new ArrayList<>(), new CalculationContext(pathomatic));
        }
    }
    ...
    private BlockOptionalMetaLookup filterFilter() {
        ...
        if (!Pathomatic.settings().allowBreak.value) {
            BlockOptionalMetaLookup f = new BlockOptionalMetaLookup(this.filter.blocks()
                    .stream()
                    .filter(e -> Pathomatic.settings().allowBreakAnyway.value.contains(e.getBlock()))
