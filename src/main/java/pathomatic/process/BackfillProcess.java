...
public final class BackfillProcess extends PathomaticProcessHelper {

    public HashMap<BlockPos, BlockState> blocksToReplace = new HashMap<>();

    public BackfillProcess(Pathomatic pathomatic) {
        super(pathomatic);
    }
    ...
    public boolean isActive() {
        ...
        if (!Pathomatic.settings().backfill.value) {
            return false;
        }
        if (Pathomatic.settings().allowParkour.value) {
            logDirect("Backfill cannot be used with allowParkour true");
            Pathomatic.settings().backfill.value = false;
            return false;
        }
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        ...
        for (BlockPos toPlace : toFillIn()) {
            ...
            switch (MovementHelper.attemptToPlaceABlock(fake, pathomatic, toPlace, false, false)) {
                case NO_OPTION:
                    continue;
                case READY_TO_PLACE:
                ...
                case ATTEMPTING:
                    // patience
                    pathomatic.getLookBehavior().updateTarget(fake.getTarget().getRotation().get(), true);
                    return new PathingCommand(null, PathingCommandType.REQUEST_PAUSE);
                default:
    ...
    private void amIBreakingABlockHMMMMMMM() {
        if (!ctx.getSelectedBlock().isPresent() || !pathomatic.getPathingBehavior().isPathing()) {
            return;
        }
    ...
    public List<BlockPos> toFillIn() {
        return blocksToReplace
                ...
                .filter(pos -> pathomatic.getBuilderProcess().placementPlausible(pos, Blocks.DIRT.defaultBlockState()))
    ...
    private boolean partOfCurrentMovement(BlockPos pos) {
        PathExecutor exec = pathomatic.getPathingBehavior().getCurrent();
