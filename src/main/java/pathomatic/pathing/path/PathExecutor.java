...
import pathomatic.pathing.movement.movements.*;
...
import static pathomatic.api.pathing.movement.MovementStatus.*;
...
public class PathExecutor implements IPathExecutor, Helper {
     ...
     *
     * @see <a href="https://github.com/shadvls/pathomatic/issues/102">Issue #102</a>
    ...
    public boolean onTick() {
        ...
        if (pathPosition < path.movements().size() - 1) {
            ...
            if (!behavior.pathomatic.bsi.worldContainsLoadedChunk(next.getDest().x, next.getDest().z)) {
        ...
        if (costEstimateIndex == null || costEstimateIndex != pathPosition) {
            ...
            for (int i = 1; i < Pathomatic.settings().costVerificationLookahead.value && pathPosition + i < path.length() - 1; i++) {
        ...
        if (!movement.calculatedWhileLoaded() && currentCost - currentMovementOriginalCostEstimate > Pathomatic.settings().maxCostIncrease.value && canCancel) {
        ...
        if (movementStatus == SUCCESS) {
            ...
            ticksOnCurrent++;
            if (ticksOnCurrent > currentMovementOriginalCostEstimate + Pathomatic.settings().movementTimeoutTicks.value) {
    ...
    private boolean shouldSprintNextTick() {
        boolean requested = behavior.pathomatic.getInputOverrideHandler().isInputForcedDown(Input.SPRINT);

        // we'll take it from here, no need for minecraft to see we're holding down control and sprint for us
        behavior.pathomatic.getInputOverrideHandler().setInputForceState(Input.SPRINT, false);

        // first and foremost, if allowSprint is off, or if we don't have enough hunger, don't try and sprint
        if (!new CalculationContext(behavior.pathomatic, false).canSprint) {
            return false;
        }
        ...
        if (current instanceof MovementTraverse && pathPosition < path.length() - 3) {
            ...
            if (next instanceof MovementAscend && sprintableAscend(ctx, (MovementTraverse) current, (MovementAscend) next, path.movements().get(pathPosition + 2))) {
                if (skipNow(ctx, current)) {
                    ...
                    onChangeInPathPosition();
                    onTick();
                    behavior.pathomatic.getInputOverrideHandler().setInputForceState(Input.JUMP, true);
                    return true;
                } else {
            ...
            if (pathPosition < path.length() - 2) {
                ...
                if (MovementHelper.canUseFrostWalker(ctx, next.getDest().below())) {
                    ...
                    if (next instanceof MovementTraverse || next instanceof MovementParkour) {
                        boolean couldPlaceInstead = Pathomatic.settings().allowPlace.value && behavior.pathomatic.getInventoryBehavior().hasGenericThrowaway() && next instanceof MovementParkour; // traverse doesn't react fast enough
        ...
        if (current instanceof MovementAscend && pathPosition != 0) {
            ...
            if (prev instanceof MovementDescend && prev.getDirection().above().equals(current.getDirection().below())) {
                ...
                if (ctx.player().position().y >= center.getY() - 0.07) {
                    behavior.pathomatic.getInputOverrideHandler().setInputForceState(Input.JUMP, false);
                    return true;
                }
            }
        ...
        if (current instanceof MovementFall) {
            ...
            if (data != null) {
                ...
                clearKeys();
                behavior.pathomatic.getLookBehavior().updateTarget(RotationUtils.calcRotationFromVec3d(ctx.playerHead(), data.getA(), ctx.playerRotations()), false);
                behavior.pathomatic.getInputOverrideHandler().setInputForceState(Input.MOVE_FORWARD, true);
                return true;
            }
        }
        return false;
    ...
    private static boolean sprintableAscend(IPlayerContext ctx, MovementTraverse current, MovementAscend next, IMovement nextnext) {
        if (!Pathomatic.settings().sprintAscends.value) {
            return false;
        }
    ...
    private static boolean canSprintFromDescendInto(IPlayerContext ctx, IMovement current, IMovement next) {
        ...
        return next instanceof MovementDiagonal && Pathomatic.settings().allowOvershootDiagonalDescend.value;
    }
    ...
    private void clearKeys() {
        ...
        behavior.pathomatic.getInputOverrideHandler().clearAllKeys();
    }

    private void cancel() {
        clearKeys();
        behavior.pathomatic.getInputOverrideHandler().getBlockBreakHelper().stopBreakingBlock();
    ...
    private PathExecutor cutIfTooLong() {
        if (pathPosition > Pathomatic.settings().maxPathHistoryLength.value) {
            int cutoffAmt = Pathomatic.settings().pathHistoryCutoffAmount.value;
