...
package pathomatic.pathing.calc;
...
import pathomatic.pathing.calc.openset.BinaryHeapOpenSet;
import pathomatic.pathing.movement.CalculationContext;
import pathomatic.pathing.movement.Moves;
...
public final class AStarPathFinder extends AbstractNodeCostSearch {
    ...
    protected Optional<IPath> calculate0(long primaryTimeout, long failureTimeout) {
        ...
        boolean slowPath = Pathomatic.settings().slowPath.value;
        if (slowPath) {
            logDebug("slowPath is on, path timeout will be " + Pathomatic.settings().slowPathTimeoutMS.value + "ms instead of " + primaryTimeout + "ms");
        }
        long primaryTimeoutTime = startTime + (slowPath ? Pathomatic.settings().slowPathTimeoutMS.value : primaryTimeout);
        long failureTimeoutTime = startTime + (slowPath ? Pathomatic.settings().slowPathTimeoutMS.value : failureTimeout);
        ...
        int pathingMaxChunkBorderFetch = Pathomatic.settings().pathingMaxChunkBorderFetch.value; // grab all settings beforehand so that changing settings during pathing doesn't cause a crash or unpredictable behavior
        double minimumImprovement = Pathomatic.settings().minimumImprovementRepropagation.value ? MIN_IMPROVEMENT : 0;
        ...
        while (!openSet.isEmpty() && numEmptyChunk < pathingMaxChunkBorderFetch && !cancelRequested) {
            ...
            if (slowPath) {
                try {
                    Thread.sleep(Pathomatic.settings().slowPathTimeDelayMS.value);
