...
public abstract class Movement implements IMovement, MovementHelper {
    ...
    protected final IPathomatic pathomatic;
    ...
    protected Movement(IPathomatic pathomatic, BetterBlockPos src, BetterBlockPos dest, BetterBlockPos[] toBreak, BetterBlockPos toPlace) {
    ...
    protected Movement(IPathomatic pathomatic, BetterBlockPos src, BetterBlockPos dest, BetterBlockPos[] toBreak) {
        this(pathomatic, src, dest, toBreak, null);
    }
    ...
    protected boolean playerInValidPosition() {
        return getValidPositions().contains(ctx.playerFeet()) || getValidPositions().contains(((PathingBehavior) pathomatic.getPathingBehavior()).pathStart());
    }
    ...
    public MovementStatus update() {
        ...
        currentState.getTarget().getRotation().ifPresent(rotation ->
                pathomatic.getLookBehavior().updateTarget(
                        rotation,
                        currentState.getTarget().hasToForceRotations()));
        pathomatic.getInputOverrideHandler().clearAllKeys();
        currentState.getInputStates().forEach((input, forced) -> {
            pathomatic.getInputOverrideHandler().setInputForceState(input, forced);
        });
    ...
    protected boolean prepared(MovementState state) {
        ...
        for (BetterBlockPos blockPos : positionsToBreak) {
            if (!ctx.world().getEntitiesOfClass(FallingBlockEntity.class, new AABB(0, 0, 0, 1, 1.1, 1).move(blockPos)).isEmpty() && Pathomatic.settings().pauseMiningForFallingBlocks.value) {
                return false;
            }
