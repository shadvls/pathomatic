...
public class MovementParkour extends Movement {
    ...
    private MovementParkour(IPathomatic pathomatic, BetterBlockPos src, int dist, Direction dir, boolean ascend) {
        super(pathomatic, src, src.relative(dir, dist).above(ascend ? 1 : 0), EMPTY, src.relative(dir, dist).below(ascend ? 0 : 1));
        this.direction = dir;
        this.dist = dist;
        this.ascend = ascend;
    }
    ...
    public static MovementParkour cost(CalculationContext context, BetterBlockPos src, Direction direction) {
        ...
        return new MovementParkour(context.getPathomatic(), src, dist, direction, res.y > src.y);
    }
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (Pathomatic.settings().allowWalkOnMagmaBlocks.value && ctx.world().getBlockState(ctx.playerFeet().below()).is(Blocks.MAGMA_BLOCK)) {
        ...
        if (ctx.playerFeet().equals(dest)) {
            ...
            if (ctx.playerFeet().equals(src.relative(direction)) || ctx.player().position().y - src.y > 0.0001) {
                if (Pathomatic.settings().allowPlace.value // see PR #3775
                        && ((Pathomatic) pathomatic).getInventoryBehavior().hasGenericThrowaway()
                        && !MovementHelper.canWalkOn(ctx, dest.below())
                        && !ctx.player().onGround()
                        && MovementHelper.attemptToPlaceABlock(state, pathomatic, dest.below(), true, false) == PlaceResult.READY_TO_PLACE
                ) {
