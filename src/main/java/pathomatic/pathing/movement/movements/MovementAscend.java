...
public class MovementAscend extends Movement {
    ...
    public MovementAscend(IPathomatic pathomatic, BetterBlockPos src, BetterBlockPos dest) {
        super(pathomatic, src, dest, new BetterBlockPos[]{dest, src.above(2), dest.above()}, dest.below());
    }
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (!MovementHelper.canWalkOn(ctx, positionToPlace, jumpingOnto)) {
            ...
            if (MovementHelper.attemptToPlaceABlock(state, pathomatic, dest.below(), false, true) == PlaceResult.READY_TO_PLACE) {
        ...
        state.setInput(Input.SNEAK, Pathomatic.settings().allowWalkOnMagmaBlocks.value && jumpingOnto.is(Blocks.MAGMA_BLOCK));
        ...
        if (Pathomatic.settings().assumeStep.value || ctx.playerFeet().equals(src.above())) {
