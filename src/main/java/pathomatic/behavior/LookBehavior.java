...
import pathomatic.behavior.look.ForkableRandom;
...
public final class LookBehavior extends Behavior implements ILookBehavior {
    ...
    public LookBehavior(Pathomatic pathomatic) {
        super(pathomatic);
        this.processor = new AimProcessor(pathomatic.getPlayerContext());
        ...
        switch (event.getState()) {
            ...
            case POST: {
                ...
                if (this.prevRotation != null) {
                    ...
                    while (this.smoothYawBuffer.size() > Pathomatic.settings().smoothLookTicks.value) {
                        this.smoothYawBuffer.removeFirst();
                    }
                    this.smoothPitchBuffer.addLast(this.target.rotation.getPitch());
                    while (this.smoothPitchBuffer.size() > Pathomatic.settings().smoothLookTicks.value) {
                    ...
                    } else if (ctx.player().isFallFlying() ? Pathomatic.settings().elytraSmoothLook.value : Pathomatic.settings().smoothLook.value) {
    ...
    public Optional<Rotation> getEffectiveRotation() {
        if (Pathomatic.settings().freeLook.value) {
    ...
    private static abstract class AbstractAimProcessor implements ITickableAimProcessor {
        ...
        public final void tick() {
            // randomLooking
            this.randomYawOffset = (this.rand.nextDouble() - 0.5) * Pathomatic.settings().randomLooking.value;
            this.randomPitchOffset = (this.rand.nextDouble() - 0.5) * Pathomatic.settings().randomLooking.value;
            ...
            this.randomYawOffset += random * Pathomatic.settings().randomLooking113.value;
        }
        ...
        enum Mode {
            ...
            static Mode resolve(IPlayerContext ctx, boolean blockInteract) {
                ...
                if (ctx.player().isFallFlying()) {
                    ...
                    // rotation needs to be set somehow, otherwise Pathomatic will halt since objectMouseOver() will just be
