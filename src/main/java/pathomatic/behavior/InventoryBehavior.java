...
public final class InventoryBehavior extends Behavior implements Helper {
    ...
    public InventoryBehavior(Pathomatic pathomatic) {
        super(pathomatic);
    }
    ...
    private boolean requestSwapWithHotBar(int inInventory, int inHotbar) {
        ...
        if (ticksSinceLastInventoryMove < Pathomatic.settings().ticksBetweenInventoryMoves.value) {
            logDebug("Inventory move requested but delaying " + ticksSinceLastInventoryMove + " " + Pathomatic.settings().ticksBetweenInventoryMoves.value);
            return false;
        }
        if (Pathomatic.settings().inventoryMoveOnlyIfStationary.value && !pathomatic.getInventoryPauserProcess().stationaryForInventoryMove()) {
    ...
    private int firstValidThrowaway() { // TODO offhand idk
        ...
        for (int i = 0; i < invy.size(); i++) {
            if (Pathomatic.settings().acceptableThrowawayItems.value.contains(invy.get(i).getItem())) {
                return i;
            }
        }
        return -1;
    ...
    private int bestToolAgainst(Block against, Class<? extends DiggerItem> cla$$) {
        ...
        for (int i = 0; i < invy.size(); i++) {
            ...
            if (Pathomatic.settings().itemSaver.value && (stack.getDamageValue() + Pathomatic.settings().itemSaverThreshold.value) >= stack.getMaxDamage() && stack.getMaxDamage() > 1) {
                continue;
            }
    ...
    public boolean hasGenericThrowaway() {
        for (Item item : Pathomatic.settings().acceptableThrowawayItems.value) {
    ...
    public boolean selectThrowawayForLocation(boolean select, int x, int y, int z) {
        BlockState maybe = pathomatic.getBuilderProcess().placeAt(x, y, z, pathomatic.bsi.get0(x, y, z));
    ...
    public boolean throwaway(boolean select, Predicate<? super ItemStack> desired) {
        return throwaway(select, desired, Pathomatic.settings().allowInventory.value);
    }
