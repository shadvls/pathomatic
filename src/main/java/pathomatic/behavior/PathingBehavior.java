...
import pathomatic.api.utils.PathCalculationResult;
import pathomatic.api.utils.interfaces.IGoalRenderPos;
import pathomatic.pathing.calc.AStarPathFinder;
...
public final class PathingBehavior extends Behavior implements IPathingBehavior, Helper {
    ...
    public PathingBehavior(Pathomatic pathomatic) {
        super(pathomatic);
    }
    ...
    private void dispatchEvents() {
        ...
        for (PathEvent event : curr) {
            pathomatic.getGameEventHandler().onPathEvent(event);
        }
    }
    ...
    public void onTick(TickEvent event) {
        ...
        if (event.getType() == TickEvent.Type.OUT) {
            ...
            pathomatic.getPathingControlManager().cancelEverything();
            return;
        }

        expectedSegmentStart = pathStart();
        pathomatic.getPathingControlManager().preTick();
        tickPath();
        ticksElapsedSoFar++;
        dispatchEvents();
    }
    ...
    private void tickPath() {
        ...
        if (pauseRequestedLastTick && safeToCancel) {
            ...
            if (unpausedLastTick) {
                ...
                pathomatic.getInputOverrideHandler().getBlockBreakHelper().stopBreakingBlock();
            }
        ...
        synchronized (pathPlanLock) {
            ...
            if (current.failed() || current.finished()) {
                ...
                if (goal == null || goal.isInGoal(ctx.playerFeet())) {
                    ...
                    next = null;
                    if (Pathomatic.settings().disconnectOnArrival.value) {
            ...
            if (Pathomatic.settings().splicePath.value) {
            ...
            synchronized (pathCalcLock) {
                ...
                if (ticksRemainingInSegment(false).get() < Pathomatic.settings().planningTickLookahead.value) {
    ...
    public boolean secretInternalSetGoalAndPath(PathingCommand command) {
        ...
        if (command instanceof PathingCommandContext) {
            ...
            context = new CalculationContext(pathomatic, true);
        }
        if (goal == null) {
            return false;
        }
    ...
    public boolean isSafeToCancel() {
        if (current == null) {
            return !pathomatic.getElytraProcess().isActive() || pathomatic.getElytraProcess().isSafeToCancel();
        }
        return safeToCancel;
    }
    ...
    public boolean cancelEverything() {
        ...
        pathomatic.getPathingControlManager().cancelEverything(); // regardless of if we can stop the current segment, we can still stop the processes
        return doIt;
    }
    ...
    private void findPathInNewThread(final BlockPos start, final boolean talkAboutIt, CalculationContext context) {
        ...
        long failureTimeout;
        if (current == null) {
            primaryTimeout = Pathomatic.settings().primaryTimeoutMS.value;
            failureTimeout = Pathomatic.settings().failureTimeoutMS.value;
        } else {
            primaryTimeout = Pathomatic.settings().planAheadPrimaryTimeoutMS.value;
            failureTimeout = Pathomatic.settings().planAheadFailureTimeoutMS.value;
        }
    ...
    private AbstractNodeCostSearch createPathfinder(BlockPos start, Goal goal, IPath previous, CalculationContext context) {
        ...
        if (Pathomatic.settings().simplifyUnloadedYCoord.value && goal instanceof IGoalRenderPos) {
            BlockPos pos = ((IGoalRenderPos) goal).getGoalPos();
            if (!context.bsi.worldContainsLoadedChunk(pos.getX(), pos.getZ())) {
                transformed = new GoalXZ(pos.getX(), pos.getZ());
            }
        }
        Favoring favoring = new Favoring(context.getPathomatic().getPlayerContext(), previous, context);
        BetterBlockPos feet = ctx.playerFeet();
