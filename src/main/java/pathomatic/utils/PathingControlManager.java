...
public class PathingControlManager implements IPathingControlManager {
    ...
    private final HashSet<IPathomaticProcess> processes; // unGh
    private final List<IPathomaticProcess> active;
    private IPathomaticProcess inControlLastTick;
    private IPathomaticProcess inControlThisTick;
    private PathingCommand command;

    public PathingControlManager(Pathomatic pathomatic) {
        this.pathomatic = pathomatic;
        this.processes = new HashSet<>();
        this.active = new ArrayList<>();
        pathomatic.getGameEventHandler().registerEventListener(new AbstractGameEventListener() { // needs to be after all behavior ticks
            @Override
    ...
    @Override
    public void registerProcess(IPathomaticProcess process) {
    ...
    public void cancelEverything() { // called by PathingBehavior on TickEvent Type OUT
        ...
        command = null;
        active.clear();
        for (IPathomaticProcess proc : processes) {
            proc.onLostControl();
    ...
    @Override
    public Optional<IPathomaticProcess> mostRecentInControl() {
    ...
    public void preTick() {
        ...
        inControlThisTick = null;
        PathingBehavior p = pathomatic.getPathingBehavior();
    ...
    private void postTick() {
        ...
        switch (command.commandType) {
            case FORCE_REVALIDATE_GOAL_AND_PATH:
                ...
                break;
            case REVALIDATE_GOAL_AND_PATH:
                if (Pathomatic.settings().cancelOnGoalInvalidation.value && (command.goal == null || revalidateGoal(command.goal))) {
                    p.softCancelIfSafe();
                }
                p.secretInternalSetGoalAndPath(command);
                break;
    ...
    public boolean forceRevalidate(Goal newGoal) {
        PathExecutor current = pathomatic.getPathingBehavior().getCurrent();
        if (current != null) {
    ...
    public PathingCommand executeProcesses() {
        for (IPathomaticProcess process : processes) {
            if (process.isActive()) {
        ...
        active.sort(Comparator.comparingDouble(IPathomaticProcess::priority).reversed());

        Iterator<IPathomaticProcess> iterator = active.iterator();
        while (iterator.hasNext()) {
            IPathomaticProcess proc = iterator.next();

            PathingCommand exec = proc.onTick(Objects.equals(proc, inControlLastTick) && pathomatic.getPathingBehavior().calcFailedLastTick(), pathomatic.getPathingBehavior().isSafeToCancel());
            if (exec == null) {
                if (proc.isActive()) {
                ...
                if (!proc.isTemporary()) {
                    iterator.forEachRemaining(IPathomaticProcess::onLostControl);
                }
                return exec;
            }
        }
