...
public class GuiClick extends Screen implements Helper {
    ...
    public void render(GuiGraphics stack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (near != null && far != null) {
            ...
            LocalPlayer player = PathomaticAPI.getProvider().getPrimaryPathomatic().getPlayerContext().player();
        ...
        if (currentMouseOver != null) { //Catch this, or else a click into void will result in a crash
            if (mouseButton == 0) {
                if (clickStart != null && !clickStart.equals(currentMouseOver)) {
                    PathomaticAPI.getProvider().getPrimaryPathomatic().getSelectionManager().removeAllSelections();
                    PathomaticAPI.getProvider().getPrimaryPathomatic().getSelectionManager().addSelection(BetterBlockPos.from(clickStart), BetterBlockPos.from(currentMouseOver));
                    MutableComponent component = Component.literal("Selection made! For usage: " + Pathomatic.settings().prefix.value + "help sel");
                    ...
                    PathomaticAPI.getProvider().getPrimaryPathomatic().getCustomGoalProcess().setGoalAndPath(new GoalBlock(currentMouseOver));
                }
            } else if (mouseButton == 1) {
                PathomaticAPI.getProvider().getPrimaryPathomatic().getCustomGoalProcess().setGoalAndPath(new GoalBlock(currentMouseOver.above()));
            }
        }
        clickStart = null;
    ...
    public void onRender(PoseStack modelViewStack, Matrix4f projectionMatrix) {
        ...
        if (currentMouseOver != null) {
            ...
            if (clickStart != null && !clickStart.equals(currentMouseOver)) {
                BufferBuilder bufferBuilder = IRenderer.startLines(Color.RED, Pathomatic.settings().pathRenderLineWidthPixels.value, true);
