...
public class ElytraCommand extends Command {
    ...
    public ElytraCommand(IPathomatic pathomatic) {
        super(pathomatic, "elytra");
    }

    @Override
    public void execute(String label, IArgConsumer args) throws CommandException {
        final ICustomGoalProcess customGoalProcess = pathomatic.getCustomGoalProcess();
        final IElytraProcess elytra = pathomatic.getElytraProcess();
        ...
        if (!args.hasAny()) {
            if (Pathomatic.settings().elytraTermsAccepted.value) {
                if (detectOn2b2t()) {
                    warn2b2t();
                }
            } else {
    ...
    private void warn2b2t() {
        if (Pathomatic.settings().elytraPredictTerrain.value) {
            long seed = Pathomatic.settings().elytraNetherSeed.value;
    ...
    private Component suggest2b2tSeeds() {
        ...
        clippy.append("Within a few hundred blocks of spawn/axis/highways/etc, the terrain is too fragmented to be predictable. Pathomatic Elytra will still work, just with backtracking. ");
        ...
        olderSeed.setStyle(olderSeed.getStyle().withUnderlined(true).withBold(true).withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal(Pathomatic.settings().prefix.value + "set elytraNetherSeed " + OLD_2B2T_SEED))).withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, FORCE_COMMAND_PREFIX + "set elytraNetherSeed " + OLD_2B2T_SEED)));
        ...
        newerSeed.setStyle(newerSeed.getStyle().withUnderlined(true).withBold(true).withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal(Pathomatic.settings().prefix.value + "set elytraNetherSeed " + NEW_2B2T_SEED))).withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, FORCE_COMMAND_PREFIX + "set elytraNetherSeed " + NEW_2B2T_SEED)));
    ...
    private void gatekeep() {
        ...
        gatekeep.append("Pathomatic Elytra is an experimental feature. It is intended for long distance travel in the Nether but will also work in the Overworld, using fireworks for vanilla boost. It will not work with any other mods (\"hacks\") for non-vanilla boost. ");
        MutableComponent gatekeep2 = Component.literal("If you want Pathomatic to attempt to take off from the ground for you, you can enable the elytraAutoJump setting (not advisable on laggy servers!). ");
        gatekeep2.setStyle(gatekeep2.getStyle().withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal(Pathomatic.settings().prefix.value + "set elytraAutoJump true"))));
        gatekeep.append(gatekeep2);
        MutableComponent gatekeep3 = Component.literal("If you want Pathomatic to go slower, enable the elytraConserveFireworks setting and/or decrease the elytraFireworkSpeed setting. ");
        gatekeep3.setStyle(gatekeep3.getStyle().withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal(Pathomatic.settings().prefix.value + "set elytraConserveFireworks true\n" + Pathomatic.settings().prefix.value + "set elytraFireworkSpeed 0.6\n(the 0.6 number is just an example, tweak to your liking)"))));
        gatekeep.append(gatekeep3);
        MutableComponent gatekeep4 = Component.literal("Pathomatic Elytra for use in the ");
        ...
        if (detectOn2b2t()) {
            ...
            if (!Pathomatic.settings().elytraPredictTerrain.value) {
                gatekeep5.append(Pathomatic.settings().prefix.value + "elytraPredictTerrain is currently disabled. ");
            } else {
                if (Pathomatic.settings().elytraNetherSeed.value == NEW_2B2T_SEED) {
                    gatekeep5.append("You are using the newer seed. ");
                } else if (Pathomatic.settings().elytraNetherSeed.value == OLD_2B2T_SEED) {
                    gatekeep5.append("You are using the older seed. ");
                } else {
                    gatekeep5.append("Defaulting to the newer seed. ");
                    Pathomatic.settings().elytraNetherSeed.value = NEW_2B2T_SEED;
                }
            }
            gatekeep.append(gatekeep5);
        } else {
            if (Pathomatic.settings().elytraNetherSeed.value == NEW_2B2T_SEED) {
                MutableComponent gatekeep5 = Component.literal("Pathomatic doesn't know the seed of your world. Set it with: " + Pathomatic.settings().prefix.value + "set elytraNetherSeed seedgoeshere\n");
                gatekeep5.append("For the time being, elytraPredictTerrain is defaulting to false since the seed is unknown.");
                gatekeep.append(gatekeep5);
                Pathomatic.settings().elytraPredictTerrain.value = false;
            } else {
                if (Pathomatic.settings().elytraPredictTerrain.value) {
                    MutableComponent gatekeep5 = Component.literal("Pathomatic Elytra is predicting terrain assuming that " + Pathomatic.settings().elytraNetherSeed.value + " is the correct seed. Change that with " + Pathomatic.settings().prefix.value + "set elytraNetherSeed seedgoeshere, or disable it with " + Pathomatic.settings().prefix.value + "set elytraPredictTerrain false");
                    gatekeep.append(gatekeep5);
                } else {
                    MutableComponent gatekeep5 = Component.literal("Pathomatic Elytra is not predicting terrain. If you don't know the seed, this is the correct thing to do. If you do know the seed, input it with " + Pathomatic.settings().prefix.value + "set elytraNetherSeed seedgoeshere, and then enable it with " + Pathomatic.settings().prefix.value + "set elytraPredictTerrain true");
                    gatekeep.append(gatekeep5);
                }
            }
        }
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The elytra command tells pathomatic to, in the nether, automatically fly to the current goal.",
                "",
                "Usage:",
                ...
                "> elytra supported - Tells you if pathomatic ships a native library that is compatible with your PC."
        );
    }
