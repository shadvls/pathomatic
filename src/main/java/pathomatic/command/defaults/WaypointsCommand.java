...
import pathomatic.api.command.datatypes.ForWaypoints;
...
public class WaypointsCommand extends Command {
    ...
    public WaypointsCommand(IPathomatic pathomatic) {
        super(pathomatic, "waypoints", "waypoint", "wp");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (action == Action.LIST) {
            ...
            IWaypoint[] waypoints = tag != null
                    ? ForWaypoints.getWaypointsByTag(this.pathomatic, tag)
                    : ForWaypoints.getWaypoints(this.pathomatic);
            ...
            ForWaypoints.waypoints(this.pathomatic).addWaypoint(waypoint);
            ...
            IWaypoint[] waypoints = ForWaypoints.getWaypointsByTag(this.pathomatic, tag);
            for (IWaypoint waypoint : waypoints) {
                ForWaypoints.waypoints(this.pathomatic).removeWaypoint(waypoint);
            }
            deletedWaypoints.computeIfAbsent(pathomatic.getWorldProvider().getCurrentWorld(), k -> new ArrayList<>()).addAll(Arrays.<IWaypoint>asList(waypoints));
            ...
            List<IWaypoint> deletedWaypoints = this.deletedWaypoints.getOrDefault(pathomatic.getWorldProvider().getCurrentWorld(), Collections.emptyList());
            ...
            waypoints.forEach(ForWaypoints.waypoints(this.pathomatic)::addWaypoint);
            ...
            if (waypoint == null) {
                ...
                if (action == Action.INFO) {
                    ...
                    recreateComponent.setStyle(recreateComponent.getStyle().withClickEvent(new ClickEvent(
                            ...
                            String.format(
                                    "%s%s save %s %s %s %s %s",
                                    Pathomatic.settings().prefix.value, // This uses the normal prefix because it is run by the user.
                                    label,
                                    waypoint.getTag().getName(),
                                    waypoint.getName(),
                    ...
                    ForWaypoints.waypoints(this.pathomatic).removeWaypoint(waypoint);
                    deletedWaypoints.computeIfAbsent(pathomatic.getWorldProvider().getCurrentWorld(), k -> new ArrayList<>()).add(waypoint);
                    ...
                    pathomatic.getCustomGoalProcess().setGoal(goal);
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The waypoint command allows you to manage Pathomatic's waypoints.",
                "",
