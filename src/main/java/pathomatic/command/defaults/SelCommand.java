...
import pathomatic.api.command.datatypes.ForAxis;
import pathomatic.api.command.datatypes.ForBlockOptionalMeta;
import pathomatic.api.command.datatypes.ForDirection;
...
import pathomatic.api.schematic.*;
import pathomatic.api.schematic.mask.shape.CylinderMask;
import pathomatic.api.schematic.mask.shape.SphereMask;
import pathomatic.api.selection.ISelection;
...
import pathomatic.utils.schematic.StaticSchematic;
...
public class SelCommand extends Command {
    ...
    private ISelectionManager manager = pathomatic.getSelectionManager();
    private BetterBlockPos pos1 = null;
    private ISchematic clipboard = null;
    private Vec3i clipboardOffset = null;

    public SelCommand(IPathomatic pathomatic) {
        super(pathomatic, "sel", "selection", "s");
        pathomatic.getGameEventHandler().registerEventListener(new AbstractGameEventListener() {
            @Override
            public void onRenderPass(RenderEvent event) {
                if (!Pathomatic.settings().renderSelectionCorners.value || pos1 == null) {
                    return;
                }
                Color color = Pathomatic.settings().colorSelectionPos1.value;
                float opacity = Pathomatic.settings().selectionOpacity.value;
                float lineWidth = Pathomatic.settings().selectionLineWidth.value;
                boolean ignoreDepth = Pathomatic.settings().renderSelectionIgnoreDepth.value;
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (action == Action.POS1 || action == Action.POS2) {
            ...
            pathomatic.getBuilderProcess().build("Fill", composite, origin);
            ...
            pathomatic.getBuilderProcess().build("Fill", clipboard, pos.offset(clipboardOffset));
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The sel command allows you to manipulate Pathomatic's selections, similarly to WorldEdit.",
                "",
