...
import pathomatic.api.command.exception.CommandInvalidTypeException;
import pathomatic.api.command.helpers.Paginator;
...
import static pathomatic.api.utils.SettingsUtil.*;

public class SetCommand extends Command {

    public SetCommand(IPathomatic pathomatic) {
        super(pathomatic, "set", "setting", "settings");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (Arrays.asList("s", "save").contains(arg)) {
            SettingsUtil.save(Pathomatic.settings());
        ...
        if (Arrays.asList("load", "ld").contains(arg)) {
            ...
            // reset to defaults
            SettingsUtil.modifiedSettings(Pathomatic.settings()).forEach(Settings.Setting::reset);
            // then load from disk
            SettingsUtil.readAndApply(Pathomatic.settings(), file);
        ...
        if (paginate) {
            ...
            List<? extends Settings.Setting> toPaginate =
                    (viewModified ? SettingsUtil.modifiedSettings(Pathomatic.settings()) : Pathomatic.settings().allSettings).stream()
            ...
            Paginator.paginate(
                    ...
                    setting -> {
                        ...
                        String commandSuggestion = Pathomatic.settings().prefix.value + String.format("set %s ", setting.getName());
        ...
        Settings.Setting<?> setting = Pathomatic.settings().allSettings.stream()
        ...
        if (!doingSomething && !args.hasAny()) {
            ...
            if (resetting) {
                ...
                try {
                    SettingsUtil.parseAndApply(Pathomatic.settings(), arg, newValue);
                } catch (Throwable t) {
                    t.printStackTrace();
            ...
            if ((setting.getName().equals("chatControl") && !(Boolean) setting.value && !Pathomatic.settings().chatControlAnyway.value) ||
                    setting.getName().equals("chatControlAnyway") && !(Boolean) setting.value && !Pathomatic.settings().chatControl.value) {
        ...
        if (args.hasAny()) {
            ...
            if (args.hasExactlyOne() && !Arrays.asList("s", "save").contains(args.peekString().toLowerCase(Locale.US))) {
                if (arg.equalsIgnoreCase("reset")) {
                    ...
                    return RelativeFile.tabComplete(args, Minecraft.getInstance().gameDirectory.toPath().resolve("pathomatic").toFile());
                }
                Settings.Setting setting = Pathomatic.settings().byLowerName.get(arg.toLowerCase(Locale.US));
                if (setting != null) {
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "Using the set command, you can manage all of Pathomatic's settings. Almost every aspect is controlled by these settings - go wild!",
                "",
                "Usage:",
                ...
                "> set load [filename] - Load settings from another file in your minecraft/pathomatic"
        );
    }
}
